...
Domoticz.prototype._getUrl = function () {
    ...
    uri.directory(this.options.directory)
    if (this.options.authentication === "basic") {
        uri.username(this.options.username)
        uri.password(this.options.password)
    }
    else {
        var b64name = new Buffer(this.options.username).toString('base64')
        var b64pass = new Buffer(this.options.password).toString('base64')
        uri.addSearch("username", b64name);
        uri.addSearch("password", b64pass);
    }
    return uri;
};
...
Domoticz.prototype.setColour = function (params, callback){
    var url = this._getUrl();
    console.log("hex = ", params.hue)
    url.addSearch("type", "command");
    url.addSearch("param", 'setcolbrightnessvalue');
    url.addSearch("idx", params.idx);
    url.addSearch("hex", params.hue);
    url.addSearch("brightness", params.brightness);
    url.addSearch("iswhite", 'false');
    console.log(url.toString())
    this._request(url, callback);
}
...
Domoticz.prototype.Kelvin = function (params, callback){
    var url = this._getUrl();
    url.addSearch("param", 'setkelvinlevel');
    url.addSearch("idx", params.idx);
    url.addSearch("kelvin", params.kelvin)
    console.log(url.toString())
    this._request(url, callback);
}
...
Domoticz.prototype.changeSwitchState = function (params, callback) {
    ...
    if (params.type == 'dimmable')
    {
        ...
        url.addSearch("level", Math.round(params.state));
